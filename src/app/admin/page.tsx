...
import { Phone, Edit2, AlertCircle, ArrowRight, Clock, Users } from "lucide-react";
...
export default function AdminDashboardPage() {
  ...
  const { data: todayAttendance = [] } = useSWR(
    ...
    async () => {
      ...
      const { data: slotsData } = await supabase
        .from("schedule_slots")
        .select("profile_id, start_time, end_time, work_location, profiles!profile_id(name, color_hex)")
      ...
      (logsData || []).forEach((log: any) => {
        if (!checkInMap.has(log.profile_id)) checkInMap.set(log.profile_id, log.created_at);
      });
      ...
      const items: TodayAttendanceItem[] = slotsData.map((slot: any) => {
        ...
        if (clockIn) {
          const diff = Math.floor((new Date(clockIn).getTime() - slotStart.getTime()) / 60000);
          if (diff > 10) { status = "late"; late_minutes = diff; }
          else { status = "attended"; }
        } else {
          status = now.getTime() - slotStart.getTime() >= 10 * 60 * 1000 ? "absent" : "scheduled";
        }
  ...
  const statusConfig = (item: TodayAttendanceItem) => ({
    ...
    late: { text: `지각 +${item.late_minutes}분`, color: "#F59E0B", bg: "#FFF7E6" },
    scheduled: { text: `${item.start_time.slice(0, 5)} 예정`, color: "#8B95A1", bg: "#F2F4F6" },
  ...
  return (
    <div className="max-w-3xl animate-in fade-in duration-500 pb-20">
      <header className="mb-8">
        <h1 className="text-2xl font-bold text-[#191F28] tracking-tight mb-1">대시보드</h1>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[17px] font-bold text-[#191F28]">오늘 출근 현황</h2>
        ...
        {todayAttendance.length === 0 ? (
          ...
          <div className="bg-white rounded-[24px] border border-slate-100 overflow-hidden divide-y divide-slate-50">
            {todayAttendance.map((item) => {
              ...
              return (
                <div key={`${item.profile_id}_${item.start_time}`} className="flex items-center justify-between px-4 py-3.5">
      ...
      {expiringHealthCerts.length > 0 && (
        <section className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-[17px] font-bold text-[#191F28]">보건증 만료 임박</h2>
