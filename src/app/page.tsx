...
export default async function HomePage() {
  ...
  const logData: RawLogData | null = rawLogData
    ? {
        ...
        store_name: (rawLogData.stores as unknown as { name: string } | null)?.name ?? null,
      }
    : null;
  ...
  const weeklySlots = (
    (weeklySlotsData ?? []) as Array<ScheduleSlot & { weekly_schedules: unknown }>
  ...
  return (
    <HomeClient
      ...
      stores={(storeData ?? []).filter((s: { name: string }) => s.name !== "목동")}
      logData={logData}
