...
async function fetchWorkplaces(): Promise<WorkLocation[]> {
  ...
  const [{ data: stores }, { data: positions }] = await Promise.all([
    supabase
      .from("stores")
      .select("id, name, lat, lng, is_gps_required, work_location_key, label, color, bg_color, display_order")
...
export function useWorkplaces() {
  ...
  /** store id → WorkLocation */
  const byId = Object.fromEntries(data.map((w) => [w.id, w]));

  /** 해당 근무지의 포지션 목록 (work_location_key 기준) */
  const positionsOf = (locationKey: string) =>
    byKey[locationKey]?.positions ?? [];

  /** 해당 근무지의 포지션 목록 (store_id 기준) */
  const positionsOfStore = (storeId: string) =>
    byId[storeId]?.positions ?? [];

  /** 근무지에 포지션이 있는지 여부 (work_location_key 기준) */
  const hasPositions = (locationKey: string) =>
    positionsOf(locationKey).length > 0;

  /** 근무지에 포지션이 있는지 여부 (store_id 기준) */
  const hasPositionsStore = (storeId: string) =>
    positionsOfStore(storeId).length > 0;

  return { workplaces: data, byKey, byId, positionsOf, positionsOfStore, hasPositions, hasPositionsStore, isLoading, mutate };
}
